...
def yolo_box_constructor(meta,np.ndarray[float] net_out, float w, float h, float threshold):
    ...
    for grid in range(SS):
        for b in range(B):
            coords[grid, b, 0] = (coords[grid, b, 0] + grid %  S) * grid_w
            coords[grid, b, 1] = (coords[grid, b, 1] + grid // S) * grid_h
